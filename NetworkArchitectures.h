...
public:
  DeepCNet(int dimension, int l, int k, ActivationFunction fn, BatchNorm batchNorm,
...
public:
  DeepC2(int dimension, int l, int k, ActivationFunction fn, BatchNorm batchNorm,int nInputFeatures,
         int nClasses, float p = 0.0f, int cudaDevice = -1, int nTop = 1);
};

class DeepCNiN : public SparseConvNet {
public:
  DeepCNiN(int dimension, int l, int k, ActivationFunction fn, BatchNorm batchNorm,
           int nInputFeatures, int nClasses, float p = 0.0f,
           int cudaDevice = -1, int nTop = 1);
};
...
public:
  DeepC2C2(int dimension, int l, int k, ActivationFunction fn, BatchNorm batchNorm,
           int nInputFeatures, int nClasses, float p = 0.0f,
           int cudaDevice = -1, int nTop = 1);
};
...
public:
  POFMPSparseConvNet(int dimension, int l, int k, float fmpShrink,
                     ActivationFunction fn, BatchNorm batchNorm, int nInputFeatures, int nClasses,
